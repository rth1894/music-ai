...
try:
    model = load_model("music_token_model_20250806-172457.h5")
...
def tokens_to_midi(tokens, output_path="generated.mid", step=0.125):
    ...
    time = 0
    for token in tokens:
        pitch, duration, velocity = map(float, token.split('_'))
...
tokens_to_midi(generated_notes, output_path="generated2.mid")
